...
public class Habilidades {
    ...
    private static String templateAtk(String nome, boolean direto, int valor, Personagem alvo, Personagem agente) {
        ...
        if (direto) {
            return alvo.getNome() + " é acertado, por " + nome + " de " + agente.getNome() + " recebendo " + valor + " de dano ao seu HP<br>";
        }
    ...
    public static String bolaDeFogo(Personagem agente, ArrayList<Personagem> alvo) {
        ...
        for (Personagem p : alvo) {
            if (p.getDEF() >= 1) {
                p.setDEF(p.getDEF() - dano);
                mensagem += Habilidades.templateAtk(nome, false, dano, p, agente);
            } else if (p.getDEF() <= 0) {
                p.setHP(p.getHP() - 1);
    ...
    public static String corteLaminar(Personagem agente, Personagem alvo) {
        ...
        if (verificaMorte(alvo)) {
            mensagem += matarPersonagem(alvo);
        }
    ...
    public String geraDescricao(String habilidade) {
        ...
        String descricao = switch (habilidade) {
            case Habilidades.RECUPERACAO ->
                recuperarDescricao();
            case Habilidades.CORTELAMINAR ->
                corteLaminarDesc();
            case Habilidades.BOLADEFOGO ->
                boladefogoDesc();
            case Habilidades.FORTIFICAR ->
                fortificarDesc();
            case Habilidades.ATAQUEBRUTAL ->
                ataqueBrutalDesc();
            case Habilidades.DRENARATAQUE ->
                drenarAtaqueDesc();
            case Habilidades.BOLADEFOGODRACONICA ->
                bolaDefogoDraconicaDesc();
            default ->
                "habilidade incorreta";
        };
        return descricao;
    }
    ...
    public String boladefogoDesc() {
        return "Conjura uma enorme bola flamejante, causando 1 ou 2 de dano em todos os inimigos. Custo: 2 de ataque";
    }
    ...
    public String bolaDefogoDraconicaDesc() {
        return "Conjura uma enorme bola flamejante, causando 3 de dano em todos os inimigos. Custo: 2 de ataque";
    }
